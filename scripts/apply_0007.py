"""Apply migrations/0007_agent_audit.sql to the Supabase database.
...
MIGRATION = ROOT / "migrations" / "0007_agent_audit.sql"
...
def main() -> int:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(sql)
        conn.commit()
        print("OK: 0007_agent_audit.sql applied (gemini_usage + agent_audit tables).")
    finally:
        conn.close()
    return 0
